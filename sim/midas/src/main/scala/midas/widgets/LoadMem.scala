...
class LoadMemWidget(val totalDRAMAllocated: BigInt)(implicit p: Parameters) extends Widget()(p) {
  ...
  lazy val module = new LoadMemWidgetImp(this)
}

// A crude load mem unit that writes in single beats into the destination memory system
class LoadMemWidgetImp(wrapper: LoadMemWidget)(implicit p: Parameters) extends WidgetImp(wrapper)(p) {
  val (memAXI4, edgesOut) = wrapper.toHostMemory.out.head
  //TODO: FIX ME
  val maxBurst = 8
